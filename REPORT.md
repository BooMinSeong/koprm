...
작성일 2026-09-23, 보강 2026-09-26(§3.8~3.10, 새 생성 없이 저장된 점수를 다시 읽은 분석; §3.11 DVTS는 새로 생성). 실험 기간 2026-09-20 ~ 2026-09-23, 서버는 RTX A6000 48GB 8장. 설계 문서는 `Plan.md`(§15에 라운드별 판정), 모든 표의 원본은 `data/reports/results.md`, 재현 절차는 `HANDOFF.md`에 있다. 이 문서는 동기부터 결론까지를 한 번에 읽을 수 있게 정리한 것이고, 수치는 위 문서들과 같다.
...
- **DVTS.** 생성 중에 학생의 스텝 점수로 가지를 쳐도(N=64, M=4) 현행 영어 PRM의 DVTS를 크게 앞선다(naive@64 0.822 대 0.660, weighted@64 0.786 대 0.712). 현행은 DVTS가 BoN보다 나은 곳이 없고 정답 경로를 많이 잃는 반면(pass@16 −7.4%p), 학생은 naive가 BoN보다 약 2.5%p 높다. 다만 예산 16의 weighted는 BoN보다 낮아 DVTS의 이득은 수 %p에 그친다(§3.11).
...
### 3.11 DVTS: 탐색 중에 PRM을 쓰면

**질문.** BoN 재채점은 끝난 풀이의 마지막 스텝 점수만 쓴다. 생성 도중에 스텝 점수로 가지를 치면(DVTS) 학생의 중간 스텝 점수가 쓸모 있는지, 그리고 현행 영어 PRM으로 한 DVTS보다 나은지 본다.

**설정.** 생성기 EXAONE-4.0-1.2B, 채점기 8B 학생(B_24k_soft ep3), KO MATH500 500문제. komath가 현행(영어 PRM 7B를 한국어 풀이에 직접)으로 저장해 둔 DVTS(`ENSEONG/ko-ko-math-500-test-EXAONE-4.0-1.2B-dvts`, 시드 0/42/64)와 같게 N=64, 빔 폭 M=4(서브트리 16개), 최대 40회 반복, T=0.8로 새로 돌렸다(`koprm/eval/dvts.py`, 8장 약 1시간). 한 반복에서 서브트리마다 다음 스텝 후보 4개를 뽑고, 마지막 스텝의 학생 점수가 가장 높은 후보를 남긴다. 결과 풀은 search-and-learn의 출력 그대로 서브트리마다 마지막 반복 직전의 접두 + 후보 4개, 합계 64개다(끝나지 않은 후보도 섞인다). 서브트리는 서로 독립이므로 예산 16은 16개 중 4개를 무작위로 골라 그 후보 16개를 쓰고, 추첨 200번의 문제별 평균으로 낸다. BoN은 저장된 64샘플의 앞 b개다. 구간은 문제 단위 대응 부트스트랩 95%이고, 영어 PRM과의 차이는 시드 0 기준이다.

| 예산 | 방식 | 학생 DVTS | 현행 DVTS (시드 0 / 42 / 64) | 학생 − 현행 | 학생 BoN | 현행 BoN |
|---|---|---|---|---|---|---|
| 16 | naive | **0.797** | 0.675 / 0.673 / 0.666 | +0.122 [+0.098, +0.145] | 0.772 | 0.670 |
| | weighted | 0.731 | 0.679 / 0.675 / 0.671 | +0.052 [+0.033, +0.073] | **0.754** | 0.706 |
| | maj | 0.687 | 0.665 / 0.661 / 0.659 | +0.021 [+0.004, +0.040] | 0.696 | 0.696 |
| | pass | 0.853 | 0.808 / 0.801 / 0.795 | +0.045 [+0.031, +0.060] | 0.882 | 0.882 |
| 64 | naive | **0.822** | 0.660 / 0.666 / 0.660 | +0.162 [+0.122, +0.198] | 0.796 | 0.664 |
| | weighted | 0.786 | 0.712 / 0.718 / 0.704 | +0.074 [+0.044, +0.106] | 0.774 | 0.718 |
| | maj | 0.746 | 0.704 / 0.712 / 0.694 | +0.042 [+0.016, +0.070] | 0.710 | 0.710 |
| | pass | 0.930 | 0.906 / 0.900 / 0.904 | +0.024 [+0.004, +0.042] | 0.946 | 0.946 |

**같은 채점기 안에서 DVTS − BoN.**

| 예산 | 방식 | 학생 | 현행 |
|---|---|---|---|
| 16 | naive | +0.025 [+0.002, +0.050] | +0.005 [−0.023, +0.032] |
| | weighted | −0.023 [−0.046, −0.002] | −0.027 [−0.050, −0.007] |
| | maj | −0.009 [−0.033, +0.014] | −0.031 [−0.051, −0.011] |
| | pass | −0.029 [−0.051, −0.009] | −0.074 [−0.096, −0.052] |
| 64 | naive | +0.026 [−0.002, +0.052] | −0.004 [−0.042, +0.032] |
| | weighted | +0.012 [−0.012, +0.040] | −0.006 [−0.032, +0.018] |
| | maj | +0.036 [+0.014, +0.060] | −0.006 [−0.030, +0.018] |
| | pass | −0.016 [−0.034, +0.004] | −0.040 [−0.062, −0.018] |

**DVTS에서도 학생이 현행을 크게 앞선다.** 두 예산, 세 방식 모두 유의하고, naive의 격차는 BoN(16에서 +0.102, 64에서 +0.132)보다 DVTS(+0.122, +0.162)에서 크다. 현행의 세 시드는 서로 1%p 안에 있어 이 격차는 시드 운이 아니다.

**가지치기는 학생에게만 득이 된다.** 현행은 DVTS가 BoN보다 나은 곳이 없고, 예산 16에서는 weighted·maj가 유의하게 낮다. 정답 경로도 많이 잃는다(pass@16 −7.4%p, pass@64 −4.0%p). 한국어 풀이의 중간 스텝에 대한 영어 PRM의 점수가 가지를 제대로 가르지 못한다는 뜻이다. 학생은 정답 경로를 덜 잃고(−2.9%p, −1.6%p) naive가 두 예산 모두 약 +2.5%p 오르며(예산 64는 구간이 0에 걸친다), 예산 64에서는 다수결도 +3.6%p 오른다. 다만 예산 16의 weighted는 BoN보다 2.3%p 낮다. 서브트리 4개의 후보가 같은 접두를 공유해 투표가 사실상 4표로 줄기 때문으로 읽힌다. 학생의 스텝 점수는 가지치기에 쓸 만하지만, 이 생성기와 예산에서 DVTS의 이득은 BoN 대비 수 %p에 그친다.

**비용.** 학생 DVTS의 생성 토큰은 모든 후보를 합쳐 문제당 44.2k로 BoN@64(35.5k)의 1.25배이고, 여기에 스텝마다 8B 학생을 부르는 비용이 더해진다(실행 시간의 대부분). 같은 정확도를 BoN으로 얻는 비용과의 비교는 하지 않았다.
...
- **DVTS는 한 설정뿐이다.** 생성기 EXAONE 하나, 학생 한 시드이고, 현행 쪽은 komath의 구현으로 돌린 저장 결과라 구현 차이(스텝당 토큰 상한 512, 풀이 전체 2048, 스텝 텍스트 정규화)가 섞여 있다. 두 DVTS 풀의 답 집계와 BoN은 같은 코드로 다시 계산했다.
...
- 코드 지도와 재현 순서: `HANDOFF.md`. 테스트: `.venv/bin/python -m pytest tests -q`(176개).
...
- DVTS(§3.11): `python -m koprm.eval.dvts run --n 64 --beam-width 4 --scorer data/ckpt/qwen3-8b_B_24k_soft/epoch3 --shard i --num-shards 8 ...`(GPU당 한 샤드, 8장 약 1시간), 결과 `data/eval/dvts/exaone-1.2b_q3-8b_B24k_soft_ep3_n64.shard*of8.jsonl`. 현행 DVTS와의 비교는 `scripts/dvts_compare.py`(CPU 약 10분, 답 그룹 캐시 후 수 초)이고 결과는 `data/reports/dvts_compare.json`.
