...
큰 학생(§15.9)이 이번 라운드의 답이다. Qwen3-8B 백본에 B_24k 소프트 라벨로 4장 FSDP 전체 미세조정을 하면 KO MATH500·EXAONE naive@64가 0.796으로, 현행 영어 PRM(0.664)은 물론 4.8만 풀이로 학습한 1.2B(0.744)도 넘는다. 같은 데이터의 1.2B 대비 +0.082 [+0.052, +0.114]로 유의하고, n=16에서는 번역 다리를 건넌 72B 교사(0.762)보다도 높다(0.772). 영어 PRM에서 출발한 7B는 그보다 낮고(0.748) dev가 오염되어 있어 비교에 쓸 수 없다. 스텝 단위 판정도 같이 올라간다. 한국어 ProcessBench F1이 8B 학생 0.638(4.8만에서 0.651)로 영어 PRM 7B를 직접 쓴 0.579를 넘었고, 72B 직접(0.746)만 아직 위다. 같은 8B를 B_48k 소프트로 한 번 더 돌렸지만(HSDP 2×4, 8장, 2,241스텝, 19.2시간) KO naive@64는 0.788로 2.4만과 같은 자리이고, 이득은 Qwen-3B 생성기(0.696)와 ProcessBench의 오탐 감소에만 남는다. 8B에서 데이터 확장은 이 크기에서 포화했다(§15.9e).
...
koprm/train/{model,data,loss,train}.py 학생 모델·손실(§6.1)·학습 루프
                                      ...
                                      `--fsdp`(torchrun, FSDP2 전체 미세조정, `--hsdp-replicas N`),
